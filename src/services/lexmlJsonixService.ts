...
const buscarLexmlZip = async (
  idSdlegDocumentoItemDigital: string
): Promise<string | null> => {
  const url =
    baseURL + "/sdleg-getter/documento/download/" + idSdlegDocumentoItemDigital;
...
export const buscarTextoLexmlAsJsonix = async (
  sigla: string,
  numero: string,
  ano: number
): Promise<any> => {
  ...
  if (proposicao?.idSdlegDocumentoItemDigital) {
    ...
    return xml ? converterXml2Jsonix(xml) : null;
  }
};
